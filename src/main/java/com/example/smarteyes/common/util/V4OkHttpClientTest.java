...
public class V4OkHttpClientTest {
    ...
    private static final String API_KEY = "a06c556824baf073e05151c11fc15b2b.qmQIahAJDxNYqwq8";
...
//        createImageRequest.setPrompt("画一个温顺可爱的小狗");
    ...
    public static void testSseInvoke() {
        String content = "";
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), "springboot框架是什么");
//         ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), "你能帮我查询2024年1月1日从北京南站到上海的火车票吗？");
     ...
     * 同步调用
     */
    public static void testInvoke(String question) {
        List<ChatMessage> messages = new ArrayList<>();
    ...
    private static String testAsyncInvoke() {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), "springboot是什么");
        messages.add(chatMessage);
        String requestId = String.format(requestIdTemplate, System.currentTimeMillis());
        // 函数调用参数构建部分
        List<ChatTool> chatToolList = new ArrayList<>();
